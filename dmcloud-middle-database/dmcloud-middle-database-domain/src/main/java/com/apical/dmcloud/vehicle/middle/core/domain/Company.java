...
import com.apical.dmcloud.vehicle.middle.core.CompanyNotExistException;
...
@Entity(name = "Middle.Company")
@Table(name = "cl_company")
public class Company extends AbstractVersionEntity
{
	...
	{
		String jpql = " delete from Middle.Company _company where _company.id = :id";
	...
	{
		String jpql = "select _company from Middle.Company _company where _company.name = :name";
	...
	{
		String jpql = "select _company from Middle.Company _company where _company.id = :companyId";
		Company company =  getRepository().createJpqlQuery(jpql.toString())
	...
	public static List<Company> findAllCompaniesInPage(String name, int pageCount, int pageSize)
	{
		...
		if(name != null)
		{
			jpql = "select _company from Middle.Company _company where _company.name = :name";
		...
		{
			jpql = "select _company from Middle.Company _company ";
			coms = getRepository().createJpqlQuery(jpql.toString())
					.setMaxResults(pageSize)
					.setFirstResult((pageCount -1) * pageSize)
					.list();
	...
	{
		String jpql = "select count(_company.id) from Middle.Company _company";
