...
import com.apical.dmcloud.storage.middle.core.PictureNotExistException;
...
public class Picture extends Resource
{
	...
	{
		String jpql = "delete from Middle.Picture _picture"
	...
	{
		String sql = "select 1 from Middle.Picture _picture"
				+ " where _picture.id = :id";
	...
	public static List<Picture> getByIdList(List<Long> ids)
	{
		...
		{
			String jpql = "select _picture from Middle.Picture _picture"
	...
	{
		String jpql = "select _picture.deviceId from Middle.Picture _picture"
				+ " where _picture.id=:id";
	...
	{
		String jpql = "select _picture.resourceStorageInfoId from Middle.Picture _picture"
				+ " where _picture.id=:id";
	...
	{
		String jpql = "update Middle.Picture _picture"
				+ " set _picture.filename=:filename"
				+ " where _picture.id=:id";
		int count = getRepository().createJpqlQuery(jpql.toString())
	...
	{
		String jpql = "select _picture.filename from Middle.Picture _picture"
				+ " where _picture.id=:id";
	...
	{
		String jpql = "update Middle.Picture _picture"
				+ " set _picture.description=:description"
				+ " where _picture.id=:id";
		int count = getRepository().createJpqlQuery(jpql.toString())
	...
	{
		String jpql = "select _picture.description from Middle.Picture _picture"
				+ " where _picture.id=:id";
	 ...
	 * @return 图片信息
	 */
	public static List<Picture> queryAllPicturesByDeviceIdAndShotTimerange(long deviceId,
			ResourceSearchType type, Date startDate, Date endDate)
	{
		...
		{
		case All:
			jpql = "select _picture from Middle.Picture _picture"
					+ " where _picture.deviceId=:deviceId"
					+ " and _picture.shootTime>=:begindate"
					+ " and _picture.shootTime<=:enddate"
			...
			ResourceSearchType type, Date startDate, Date endDate)
	{
		...
		case All:
			sql = "select count(id),UPLOADDATE from cl_picture"
					+ " where VEHICLE_ID=:vehicleId"
					+ " and UPLOADDATE between :beginDate and :endDate group by UPLOADDATE"
					+ " order by UPLOADDATE desc";
			break;
