package com.apical.dmcloud.driving.middle.core.domain;
...
import com.apical.dmcloud.storage.middle.core.domain.Resource;
 ...
 *
 */
@Entity(name = "Middle.DeviceDrivingRecord")
@Table(name = "cl_drivingrecord_device")
public class DeviceDrivingRecord extends AbstractIDEntity
{
	/**
	 * serialVersionUID = 1L;
	 */
	...
	{
		String jpql = "delete from Middle.DeviceDrivingRecord _record"
				+ " where _record.id=:id";
	 ...
	 * @return 添加是否成功
	 */
	public static boolean addEndPoint(long recordId, long endTime)
	{
		Date entTime = new Date(endTime);
		String jpql = "update Middle.DeviceDrivingRecord _record"
				+ " set _record.endTime=:endTime"
				+ " where _record.id=:id";
		int count = getRepository().createJpqlQuery(jpql.toString())
				.addParameter("endTime", entTime)
	 ...
	 * @return 添加是否成功
	 */
	public static boolean addResource(long recordId, long resourceId, ResourceType type)
	{
		if(type == ResourceType.Picture)
		{
			String sql = "insert into cl_drivingrecord_device_picture_map (DEV_RECORDID,PICTURE_ID)"
					+ " values(:recordId,:pictureId)";
			long count = getRepository().createSqlQuery(sql.toString())
					.addParameter("recordId", recordId)
					.addParameter("pictureId", resourceId)
					.executeUpdate();
			if(count == 1)
			{
				return true;
		...
		{
			String sql = "insert into cl_drivingrecord_device_video_map (DEV_RECORDID,VIDEO_ID)"
					+ " values(:recordId,:videoId)";
			long count = getRepository().createSqlQuery(sql.toString())
					.addParameter("recordId", recordId)
					.addParameter("videoId", resourceId)
					.executeUpdate();
			if(count == 1)
			{
				return true;
	...
	{
		String jpql = "update Middle.DeviceDrivingRecord _record"
				+ " set _record.longitude=:longitude"
				+ ",_record.latitude=:latitude"
				+ ",_record.mileage=:mileage"
				+ ",_record.minSpeed=:minSpeed"
				+ ",_record.averageSpeed=:averageSpeed"
				+ ",_record.maxSpeed=:maxSpeed"
				+ " where _record.id=:id";
		int count = getRepository().createJpqlQuery(jpql.toString())
	...
	{
		String jpql = "select count(_record.id) from Middle.DeviceDrivingRecord _record"
	...
	public static long countAllByVehicleIdAndTimerange(long vehicleId, Date startDate, Date endDate)
	{
		String jpql = "select count(_record.id) from Middle.DeviceDrivingRecord _record"
				+ " where _record.vehicleId=:vehicleId"
				+ " and _record.uploadTime > :beginDate"
				+ " and _record.uploadTime < :endDate";
		Long count = getRepository().createJpqlQuery(jpql.toString())
				.addParameter("vehicleId", vehicleId)
	...
	{
		String query = "select _record from Middle.DeviceDrivingRecord _record"
				+ " where _record.vehicleId=:vehicleId"
