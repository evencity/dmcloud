...
public class DeviceLoginRecord extends AbstractIDEntity
{
	...
	{
		String jpql = "select count(_record.id) from DeviceLoginRecord _record"
	...
	{
		String jpql = "select _record from DeviceLoginRecord _record"
				+ " where _record.deviceId=:deviceId"
	...
	/**
	 * 通过设备id和时间范围，来统计登录信息数量
	 * @param deviceId 设备id
	 * @param startDate 起始日期
	 * @param endDate 结束日期
	 * @return 数量
	 */
	public static long countAllByDeviceIdAndTimerange(long deviceId, Date startDate, Date endDate)
	{
		String jpql = "select count(_record.id) from DeviceLoginRecord _record"
				+ " where _record.deviceId=:deviceId"
	 ...
	 * @return 登录信息
	 */
	public static List<DeviceLoginRecord> queryAllByDeviceIdAndTimerangeInPage(long deviceId,
	 ...
	 * @return 在线设备数目
	 */
	public static long countOnlineDevice(){
		String jpql = "select count(_record.id) from DeviceLoginRecord _record"
	 ...
	 * @return
	 */
	public static List<DeviceLoginRecord>  queryOnlineDeviceInPage(int pageCount, int pageSize){
		String jpql = "select _record from DeviceLoginRecord _record"
				+ " where _record.logoutTime != '2016-01-01 00:00:00'";
		List<DeviceLoginRecord> records = getRepository().createJpqlQuery(jpql.toString())
				.setMaxResults(pageSize)
